...
def actor_mesh_cut(polydata, color=(1.,1.,1.), wireframe=False, origin=(0,0,0),\
                   ...
                   opacity = 0.3,opacityE = 0.7,linewidth = 2,dotted_line = 0,lesion_actor =False):
    """
    ...
    planeCutActor.GetProperty().SetLineWidth(linewidth)
    
    if dotted_line:
        planeCutActor.GetProperty().SetLineStipplePattern(0xAAAA)
        planeCutActor.GetProperty().SetLineStippleRepeatFactor(2)
    ...
    if lesion_actor == True:
        cutActorP.GetProperty().SetColor(color[0],color[1],color[2])
    else:
    ...
    if lesion_actor == True:
        cutActorN.GetProperty().SetColor(color[0],color[1],color[2])
    else:
...
def launch_vtk_slice(actors, cut_plane_normal=(0,-1,0), window_name=None, window_size=None,
...
def volume_to_imagedata_respace(arr, origin=(0,0,0), spacing = [1.,1.,1.], auxdata = None):
    ...
    if auxdata is not None:
        auxdata = np.transpose(auxdata, [2,1,0])
...
def make_scaleBar_actor_mm(O,N):
    x0 = float(O[0])
    x1 = float(O[1])
    x2 = float(O[2])
