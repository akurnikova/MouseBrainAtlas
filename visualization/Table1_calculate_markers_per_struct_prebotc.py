...
include_stacks = ['RV4','RV19','RV16','RV14','RV15']#,'RV13','RV4','RV19','RV9','RV10','RV2_L','RV2_R']#,'RV12','RV7','RV8']
...
include_structs = ['GiA','Cortex','RIP', 'ROB', 'RPa','PreBotC_L','PreBotC_R']
cell_count = {}
cc = pd.DataFrame()
#%%
markers_count = dict()
...
for OZ in [215]:# #]range(-240,200,4):
    ...
    ## Load in slice
    slice_coords_dict_atlas_top, slice_coords_dict_atlas_bot = load_atlas_slice_ranges(atlas_name = 'Rat_brainstem_atlas',cut_plane_origin = cut_plane_origin,cut_plane_normal = cut_plane_normal, spacefill = spacefill,thickness = thickness,include_structs = include_structs)
    ...
    for i, (x,y,z) in enumerate(all_markers[stack]):
        ...
        if dist < thickness:
            for name_struct in all_polygons_T.keys():
                ...
                markers_in_struct.append((i, name_struct,T,B,Tdist,Bdist,x,y))
                        
    mFr = pd.DataFrame(markers_in_struct,columns = ['ncell','name_struct','T','B','Tdist','Bdist','x','y'])
    ...
    for c in mFr['ncell'].unique():
        ...
        markers_by_struct = markers_by_struct.append(pd.DataFrame([x,y,z,name]).T,ignore_index = True)
...
if 1:
    fp = '/home/asya/Documents/Yuncong_code/data/cell_counts/Count1_' + stack + '.pckl'
    cc_stack.to_pickle(fp)
    
    markers_by_struct.columns = ['x', 'y','z','struct']
    
    fp = '/home/asya/Documents/Yuncong_code/data/cell_counts/Coords1_' + stack + '.pckl'
