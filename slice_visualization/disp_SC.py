...
fig_title = '/home/asya/Documents/Yuncong_code/Figures_code/Atlas_figs/SC_2_S.pdf'
...
for i,stack in enumerate(include_stacks):
    stack_to_color[stack] = sns.color_palette()[i+8]

fig = figure()
ax = fig.add_subplot(111)


fp = DataManager.get_mesh_filepath(stack_m='Rat_brainstem_atlas', structure='SCInG_R')
...
if dim1 == 0 and dim2 == 1: #Sagittal
    for orig in np.arange(-500,500,20):
...
fp = DataManager.get_mesh_filepath(stack_m='Rat_brainstem_atlas', structure='SCSuG_R')
mesh = load_mesh_stl(fp,return_polydata_only=True)
...
if dim1 == 0 and dim2 == 1: #Sagittal
    for orig in np.arange(-500,500,20):
        ...
        STRUCT = contour_points[order]
        STRUCT = vstack((STRUCT,STRUCT[0]))
        plt.plot(STRUCT[:,dim1],-STRUCT[:,dim2],'b')
