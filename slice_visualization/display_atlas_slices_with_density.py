...
##Coronal
cut_plane_origin = (-210,0,0)
cut_plane_normal = (1,0,0)

thickness = 5

include_stacks = {'RV14'}#'RV14'} #
...
if do_slice:
    slice_actor_dict_atlas = generate_slice_actors_atlas(cut_plane_origin = cut_plane_origin,cut_plane_normal = cut_plane_normal, thickness = thickness,\
                                doBrainstem = 1,doCortex = 0,opacity= 0.3, structs_to_plot = all_known_structures_sided, constcolor = (0.5,0.5,0.5),spacefill = spacefill,atlas_name = atlas_name)
   ...
   #                                           include_stacks=include_stacks, radius=2, stacks_to_colors = [])
    ...
    density_actor_dict = generate_density_slice_actors(cut_plane_origin = cut_plane_origin,cut_plane_normal = cut_plane_normal, thickness = thickness,\
                                opacity= 0.3, color = (1,0,1),linewidth = 3, include_stacks = include_stacks,include_d = {'0.5','0.7','0.9'})
...
if do_slice:
  ...
  #  vtk_slice_input_list.extend([a for a in slice_marker_actors_all_stacks])
    ...
    for name_stack in density_actor_dict:
        vtk_slice_input_list.extend([[] if a == [] else a[0] for a in density_actor_dict[name_stack].values()])
...
if do_volume_display:
    structs_to_plot = ['7N','IO','LRT','Amb','5N','SpVI','SpVO','SpVC','PreBotC','Brainstem']#,'SCInG','SCSuG','IC','ZI','RN','fr','scp','SNR']
